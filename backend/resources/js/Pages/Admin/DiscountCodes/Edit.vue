    ...
    <AdminLayout>
        <template #header>
            <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                <h2 class="font-semibold text-2xl text-slate-100 leading-tight">Edit Discount Code</h2>
                ...
                <div class="bg-slate-900 border border-slate-800 overflow-hidden rounded-xl">
                    <form @submit.prevent="submit" class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-400 mt-1">Leave empty for unlimited. Current usage: {{ discountCode.usage_count }}</p>
                        </div>
...
const form = useForm({
    ...
    min_purchase: props.discountCode.min_purchase ?? '',
    max_discount: props.discountCode.max_discount ?? '',
    usage_limit: props.discountCode.usage_limit ?? '',
...
const submit = () => {
    form.put(route('admin.discount-codes.update', props.discountCode.id));
};
</script>
